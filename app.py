...
def is_data_already_in_cache(
    currency: str, start_date: str, end_date: str, db_file: str
) -> bool:
...
def fetch_currency_rates(
    ...
    if response.status_code == 200:
        ...
        error_message = (
            "Error 404: No data found for selected currency and/or time frame."
        )
...
def save_currency_rates_to_db(
    currency: str, currency_rates: dict, db_file: str
) -> None:
    conn = sqlite3.connect(db_file)
    with conn:
        c = conn.cursor()
...
def validate_user_input(start_date, end_date):
    ...
    try:
        assert (
            start_date_date_format <= end_date_date_format
        ), "'Start Date' cannot be after 'End Date'."
        max_range = timedelta(days=93)
        assert (
            end_date_date_format - start_date_date_format <= max_range
        ), "Maximum date range is 93 calendar days."
...
def index():
    ...
    if request.method == "POST":
        ...
        data_already_in_cache = is_data_already_in_cache(
            selected_currency, start_date, end_date, db_file
        )

        if data_already_in_cache is False:
            error_message, currency_rates = fetch_currency_rates(
